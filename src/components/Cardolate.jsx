...
const STAR_DENSITY_DIVISOR = 1200;
const STAR_CAP_DESKTOP = 12000;
const STAR_CAP_MOBILE = 6000;
const GRAVITY_NUM = 15;
const GRAVITY_CAP = 0.2;
const DAMPING = 0.985;

export default function Cardolate({
  initialTo = "",
  initialFrom = "",
  ...
  async function handleDownloadPng() {
    try {
      ...
      const dataUrl = await toPng(clone, { cacheBust: true, pixelRatio: 2, backgroundColor: "#070b12" });
  ...
  return (
    <main className="root">
        ...
        {!onlyCard && (
          <section className="controls">
            ...
            <label className="field">
              ...
              <input className="input" value={to} onChange={(e) => setTo(e.target.value)} placeholder="Receiver’s name" disabled={readOnly} />
            </label>

            <label className="field">
              <span className="label">Message</span>
              <textarea className="input textarea" rows={6} value={message} onChange={(e) => setMessage(e.target.value)} disabled={readOnly} />
            </label>

            <label className="field">
              <span className="label">From</span>
              <input className="input" value={from} onChange={(e) => setFrom(e.target.value)} placeholder="Your name" disabled={readOnly} />
            </label>

            <div className="buttons">
              <button className="btn primary" onClick={() => setIsOpen(v => !v)}>{isOpen ? "Close" : "Open"} card</button>
              <button className="btn" onClick={handleDownloadPng}>Download PNG</button>
              {!readOnly && !existingId && <button className="btn" onClick={saveCardAndCopyLink}>Get share link</button>}
      ...
      <style jsx>{`
        ...
        .wrap{max-width:1200px;margin:0 auto;padding:20px 14px;display:grid;grid-template-columns:1fr;gap:20px;}
        @media (min-width:1024px){.wrap{padding:40px 20px;grid-template-columns:${onlyCard ? "1fr" : "1fr 1fr"};gap:32px;}}
        .controls{z-index:2;}
        ...
        .input{background:rgba(10,12,20,.6);border:1px solid rgba(255,200,120,.35);border-radius:12px;padding:12px 14px;color:#fff;outline:none;font-size:16px;}
        .textarea{min-height:120px;}
        .buttons{display:flex;gap:10px;flex-wrap:wrap;margin-top:10px;}
        .btn{background:#121521;border:1px solid #2b2f3f;color:#fff;border-radius:12px;padding:12px 14px;cursor:pointer;font-size:15px;text-decoration:none;display:inline-flex;align-items:center;}
        ...
        .card{position:relative;width:min(92vw,720px);height:calc(min(92vw,720px)*0.64);transform-style:preserve-3d;transform-origin:50% 50%;transition:transform .7s cubic-bezier(.22,.61,.36,1);cursor:pointer;}
        .card.open{transform:rotateY(180deg);}
        @media (prefers-reduced-motion:reduce){.card{transition:none;}}
        .panel{position:absolute;inset:0;border-radius:14px;overflow:hidden;backface-visibility:hidden;box-shadow:0 10px 40px rgba(0,0,0,.55);}
        @media (min-width:480px){.panel{border-radius:18px;}}
  ...
  return (
    <div className="cover">
      ...
      <style jsx>{`
        ...
        .stack{position:relative;width:420px;max-width:84%;aspect-ratio:1/1;}
        @media (min-width:480px){.stack{width:470px;max-width:88%;}}
        ...
        .core{position:absolute;inset:0;margin:auto;width:68%;height:68%;object-fit:contain;}
        .smallCore{width:64%;height:64%;}
        .footer{position:absolute;inset:auto 0 14px 0;text-align:center;color:#f7c34d;letter-spacing:.25em;font-size:10px;}
        @media (min-width:480px){.footer{font-size:12px;}}
      `}</style>
    </div>
  );
}
  ...
  return (
    <div className="inside">
      ...
      <style jsx>{`
        ...
        .innerClip{position:absolute;inset:18px;border-radius:12px;overflow:hidden;outline:1px solid rgba(247,195,77,.25);background:#0b101a;}
        ...
        .msg{position:relative;z-index:1;margin:4px 10px 40px;padding:14px;min-height:140px;background:rgba(0,0,0,.32);border:1px solid rgba(247,195,77,.2);border-radius:12px;color:#eef0f5;line-height:1.5;white-space:pre-wrap;}
...
function Starfield() {
  ...
  useEffect(() => {
    ...
    const base = Math.floor((w * h) / STAR_DENSITY_DIVISOR);
    ...
    const step = () => {
      ...
      for (const s of stars) {
        if (mouse.current.active) {
          ...
          const f = Math.min(GRAVITY_CAP, GRAVITY_NUM / d2);
  ...
  return (
    <canvas ref={canvasRef} className="starcanvas" aria-hidden>
      <style jsx>{`.starcanvas{position:fixed;inset:0;width:100%;height:100%;z-index:0;pointer-events:none;}`}</style>
    </canvas>
  );
}
